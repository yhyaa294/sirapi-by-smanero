...
echo ===================================================
echo   🚀 SMARTAPD PRODUCTION LAUNCHER
...
start "SmartAPD - BACKEND" cmd /k "cd backend && echo 📦 Cek Modul... && go mod tidy && echo 🚀 Menjalankan Server... && go run cmd/server/main.go"

:: 2. Jalankan AI Engine (Python)
echo [2/3] Menyalakan AI Engine (Port 8000)...
:: Menggunakan Python VENV agar tidak error library
start "SmartAPD - AI ENGINE" cmd /k "cd ai-engine && echo 🐍 Menggunakan VENV Python... && "..\.venv\Scripts\python.exe" web_server.py"

:: 3. Jalankan Frontend (Next.js)
echo [3/3] Menyalakan Frontend (Port 3000)...
start "SmartAPD - FRONTEND" cmd /k "cd frontend && echo 🎨 Menjalankan Dashboard... && set PORT=3000 && npm run dev"
...
echo 📹 AI Stream: http://localhost:8000/video_feed
